    ...
    if (CustomerCart[key].productid === order.productid) {
      ...
      try {
        const response = await fetch(
          `api/orderdetails/${order.orderid}/${order.productid}`,
          {
            method: "PATCH",
            headers: {
        ...
        console.error(
          `An error has occured when Adding/Subtracting the quanity ${error} : EditQuanity`
        );
      }
    }
  }
...
export async function addToCart(purchaceAmount, order, stockImg) {
  ...
  for (let key in CustomerCart) {
    if (CustomerCart[key].id === NewOrder.id) {
      CustomerCart[key].amount =
        Number(CustomerCart[key].amount) + Number(purchaceAmount);
      CustomerCart[key].total =
        Number(CustomerCart[key].price) * Number(CustomerCart[key].amount);
      OrderFound = true;
      break;
    }
  }
...
function Cart() {
  ...
  async function getUserOrders(user_Id) {
    try {
      const response = await fetch(
        `http://localhost:4000/api/orders/status/current/${user_Id}`
      );
      const result = await response.json();
        ...
        return (
          <ul key={product.productid}>
            <div id={product.orderid} className="order">
              <p id="Title" className="content">
                Title: {product.title}{" "}
              </p>
              <p id="Author" className="content">
                Author: {product.author}
              </p>
              <img src={product.imageurl} className="content"></img>
              <section id="QuanitySection">
                <p id="Quantity" className="content">
                  Quantity: {product.quantity}
                </p>
                <button
                  id="AddQuantity"
                  className="content"
                  onClick={(e) => EditQuantity(e, product)}
                >
                  +
                </button>
                <button
              ...
              <p id="Price" className="content">
                Price: ${product.itemprice}
              </p>

              <button id="remove" className="content" onClick={deleteFromCart}>
                Remove
              </button>
            </div>
          </ul>
